...
| Compare | **Compare** freezes the current selection as A; whatever you filter next is B. The answer becomes two cards, A and B, with the same figures side by side and the difference next to B (installations, Mt, share of the ETS, change vs baseline, allocation and coverage, closures, per-capita, per-GDP or national share); the sector, country, operator and region bars pair up, the trajectory shows two lines, and A stays on the map as hollow rings. The comparison is part of the link and of the PDF. |
| Outputs | **Download CSV** of the selection (45 columns, tonne precision) · **Report (PDF)**: one page with filters, KPIs or the A/B table, map, breakdowns, largest sites and the open card · **Copy link**: every filter, the comparison and the open card live in the URL |

Colour has one meaning each. The "Delta" palette (inspired by Nadieh Bremer's map of the Ebro delta): navy sea, white land, navy for installations and their CO₂ (white-edged so they read on both), cyan for *down*, *closed* and the pinned selection A, rose for *up*, and continuous cyan · navy · rose ramps for change and allocation coverage; sector colours appear only when asked. The page opens on a globe (MapLibre globe projection, native circle layer) and comes down to the map; shared links skip it. Every hue was checked with a palette validator for contrast and colour-vision deficiency on that ground.
